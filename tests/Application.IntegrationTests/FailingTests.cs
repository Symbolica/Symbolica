...
public class FailingTests
{
    ...
    private async Task ShouldFail(DirectoryInfo directory, string optimization, Options options,
    ...
    private sealed class TestData : TheoryData<DirectoryInfo, string, Options, StateError, string[]>
    {
        public TestData()
        {
            ...
            Add(DivideCases());
            Add(BufferCases());
            // Disabled because they'll run forever unless there's a limit on max errors
            // Add(GeometricCases());
        }
        ...
        private void Add(IEnumerable<(string, string, Options, StateError, string[])> cases)
        {
            foreach (var (directory, optimization, options, error, symbols) in cases)
                Add(
                    new DirectoryInfo(Path.Combine("..", "..", "..", "..", "fail", directory)),
                    optimization,
                    options,
                    error,
                    symbols);
