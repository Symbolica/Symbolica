...
public class FloatLessOrEqualTests
{
        ...
        IValue left1, IValue right1)
    {
        ...
        using var bv0 = FloatLessOrEqual.Create(left0, right0).AsBitVector(context);
        using var result0 = bv0.Simplify();

        using var bv1 = FloatLessOrEqual.Create(left1, right1).AsBitVector(context);
        ...
        IValue left1, IValue right1)
    {
        ...
        using var b0 = FloatLessOrEqual.Create(left0, right0).AsBool(context);
        using var result0 = b0.Simplify();

        using var b1 = FloatLessOrEqual.Create(left1, right1).AsBool(context);
