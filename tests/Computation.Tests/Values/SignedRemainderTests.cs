...
public class SignedRemainderTests
{
        ...
        IValue left1, IValue right1)
    {
        ...
        using var bv0 = SignedRemainder.Create(left0, right0).AsBitVector(solver);
        using var result0 = bv0.Simplify();

        using var bv1 = SignedRemainder.Create(left1, right1).AsBitVector(solver);
        ...
        IValue left1, IValue right1)
    {
        ...
        using var b0 = SignedRemainder.Create(left0, right0).AsBool(solver);
        using var result0 = b0.Simplify();

        using var b1 = SignedRemainder.Create(left1, right1).AsBool(solver);
