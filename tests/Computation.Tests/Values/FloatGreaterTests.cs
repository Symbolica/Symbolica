...
public class FloatGreaterTests
{
        ...
        IValue left1, IValue right1)
    {
        ...
        using var bv0 = FloatGreater.Create(left0, right0).AsBitVector(context);
        using var result0 = bv0.Simplify();

        using var bv1 = FloatGreater.Create(left1, right1).AsBitVector(context);
        ...
        IValue left1, IValue right1)
    {
        ...
        using var b0 = FloatGreater.Create(left0, right0).AsBool(context);
        using var result0 = b0.Simplify();

        using var b1 = FloatGreater.Create(left1, right1).AsBool(context);
