...
internal sealed record Write : BitVector
{
    ...
    {
        var writeData = ShiftLeft.Create(ZeroExtend.Create(Size, _writeValue), Resize.Create(Size, _writeOffset));
    ...
    {
        return ShiftLeft.Create(ConstantUnsigned.CreateZero(size).Not().Extend(buffer.Size), Resize.Create(buffer.Size, offset));
    }
