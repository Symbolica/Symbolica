...
public sealed record UnsignedLessOrEqual : IBinaryBoolExpression
{
    private UnsignedLessOrEqual(IExpression<IType> left, IExpression<IType> right)
    {
        Left = left;
        Right = right;
        Type = Bool.Type;
    ...
    public static IExpression<IType> Create(IExpression<IType> left, IExpression<IType> right)
    {
        ...
        {
            (IConstantValue<IType> l, IConstantValue<IType> r) => l.AsUnsigned().LessOrEqual(r.AsUnsigned()),
