...
public sealed record Xor : IBinaryBitVectorExpression
{
    private Xor(IExpression<IType> left, IExpression<IType> right)
    {
        Left = left;
        Right = right;
    ...
    private static IExpression<IType> Create(IExpression<IType> left, ConstantUnsigned right)
    {
        return left switch
        {
            IConstantValue<IType> l => l.AsUnsigned().Xor(right),
