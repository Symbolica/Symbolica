...
public sealed record Or : IBinaryBitVectorExpression
{
    private Or(IExpression<IType> left, IExpression<IType> right)
    {
        Left = left;
        Right = right;
    ...
    private static IExpression<IType> Create(IExpression<IType> left, ConstantUnsigned right)
    {
        return left switch
        {
            IConstantValue<IType> l => l.AsUnsigned().Or(right),
