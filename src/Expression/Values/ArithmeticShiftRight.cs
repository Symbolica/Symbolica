...
public sealed record ArithmeticShiftRight : IBinaryBitVectorExpression
{
    private ArithmeticShiftRight(IExpression<IType> left, IExpression<IType> right)
    {
        Left = left;
        Right = right;
    ...
    public static IExpression<IType> Create(IExpression<IType> left, IExpression<IType> right)
    {
        ...
        {
            (IConstantValue<IType> l, IConstantValue<IType> r) => l.AsSigned().ShiftRight(r.AsUnsigned()),
